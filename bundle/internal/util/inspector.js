...
const { validatePort } = require('../validators');
const permission = require('../process/permission');
...
function installConsoleExtensions(commandLineApi) {
  ...
  const { tryGetCwd } = require('../process/execution');
  const CJSModule = require('../modules/cjs/loader').Module;
  const { makeRequireFunction } = require('../modules/helpers');
