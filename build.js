...
const yauzl = require("yauzl-promise"),
  ...
  { noopify } = require("./util.js"),
  { getLatestZipball } = require("./getLatestGithubRepoRelease.js");

(async function () {
  // Get url of latest nodejs version
  const zipballData = await getLatestZipball("https://github.com/nodejs/node");
  writeFileSync("./version.txt", zipballData[0]);
  const res = zipballData[1];
  // Create TEMP
  mkdirSync("./temp");
  var zipStream = createWriteStream("./temp/nodejs.zip");
  await new Promise((r) => res.data.pipe(zipStream).once("close", r));
  finished(zipStream);
  // File created
  ...
  const patches = ["var primordials = {};", "module.exports = primordials;"];
  writeFileSync(
