...
finishedPromise.then(async () => {
 ...
 /* writeFileSync("./temp/node-main/lib/internal/util.js",readFileSync("./utilPatch.js"));
  ...
  for (const filePath of files) {
   ...
   writeFileSync(filePath,`//Patch\n//BOCK\nvar primordials = require("${primordailsLocation}");var process = require("${processLocation}");` + readFileSync(filePath).toString().replace(/(?<=require\(")internal\/[^"]*(?="\))|(?<=require\(')internal\/[^']*(?='\))/g,(match) => {var out = (filePath.includes("/") ? relative(dirname(filePath),match) : match);out = out == "" ? relative(dirname(filePath),match + ".js") : out;return out.replace(/(?<!\.\/)(?<![A-z\/\.])(?=[A-z])/,"./")}).replace(/(?<=require\('|")events(?='|"\))/gm,getLoc("events.js",filePath)))
  }
  console.log(files);
